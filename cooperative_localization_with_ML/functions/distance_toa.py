...
def value(channel: dict, max_distance_measurement: int, distance_accurate: float)-> dict:
  ...
  for toa_distance in range(max_distance_measurement):
    ...
    if is_los:
      pass_loss: float = 0.0
      shadowing: float = awgn.value(shadowing_standard_deviation)
      ...
      if rx_power > receiver_sensitivity_threthold:
        noise = awgn.value(channel["los"]["standard_deviation"])
      ...
      distance_estimated = distance_error
      break
      noise: float = awgn.value(channel["nlos"]["standard_deviation"])
  ...
  #       # noise: float = awgn.value(los["standard_deviation"]*(math.log(1.0 + estimated)))
  #       noise: float = awgn.value(los["standard_deviation"])
