...
def value(sensors_available: np.ndarray, target_localized: np.ndarray, channel: dict, max_distance_measurement: int) -> float:
    distance_from_sensor_to_target_localized = np.linalg.norm(sensors_available[:, :2] - target_localized[:2])
    distance_estimated = distance_toa.value(channel, max_distance_measurement, distance_from_sensor_to_target_localized)
