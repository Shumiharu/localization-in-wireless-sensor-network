...
def value(sensor: dict, avg_mesured: array) -> dict:
  # R 
  ...
  for i in range(len(sensor) - 1):
    I_matrix[i] = -(sensor[i]["x"])**2.0 - (sensor[i]["y"])**2.0 + (avg_mesured[i])**2.0\
       - (-(sensor[(len(sensor) - 1)]["x"])**2.0 - (sensor[(len(sensor) - 1)]["y"])**2 + (avg_mesured[(len(sensor) - 1)])**2.0)
