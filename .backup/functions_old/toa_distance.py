...
def value(target: dict, sensor: dict, los: dict, nlos: dict, max_test: int)-> array:
  avg_measured: list[float] = np.array([0.0]*len(sensor))
  for i in range(len(sensor)):
    ...
    for test in range(max_test):
      ...
      total_measured += estimated + noise + (los["avg"]*math.log(1.0 + estimated) + nlos["avg"]*sensor[i]["is_nlos"]) # 同様にlos["sigma"]もm（メートル）なのでこちらもメートルが返る．すなわち平均でlos["avg"] m分だけずれてlos["sigma"] mで細かく変動することになる．
      # total_measured += estimated + (noise + los["avg"] + nlos["avg"]*sensor[i]["is_nlos"])*math.log(1.0 + estimated)
      # total_measured += estimated
    if(total_measured) < 0.0:
      total_measured = 0.0
    avg_measured[i] = total_measured/max_test
  return avg_measured
