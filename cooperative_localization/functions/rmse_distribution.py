...
def update(field_distribution: np.ndarray, grid_interval: float, targets: np.ndarray, squared_error_list: np.ndarray):
  ...
  for target_rounded, squared_error in zip(targets_rounded, squared_error_list):
    indices: np.ndarray = np.where(np.all(np.isclose(field_distribution[:, :2], target_rounded[:2]), axis=1))[0] # これは難しい
    if indices.size > 0:
      for index in indices:
        data_count = field_distribution[index, 3]
        field_distribution[index, 2] = np.sqrt((field_distribution[index, 2]*data_count + squared_error)/(data_count + 1))
...
# field_distribution = update(field_distribution, grid_interval, targets, squared_error_list)
