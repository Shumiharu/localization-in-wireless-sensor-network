...
def calculate(channel: dict, max_distance_measurement: int, distance_accurate: float):
  tx_power: float = channel["tx_power"] #送信電力
  path_loss_at_the_reference_distance: float = channel["path_loss_at_the_reference_distance"] #自遊空間経路損失
  path_loss_exponent: float = channel["path_loss_exponent"] #パスロス指数
  receiver_sensitivity_threthold: float = channel["receiver_sensitivity_threshold"] #受信感度
  distance_error: float = channel["distance_error"] #受信感度を下回った場合（受信に失敗した場合-infを返す）
  ...
  for distance_measurement in range(max_distance_measurement):
    is_los = channel["los"]["probability"] > random.uniform(0, 1) #基本的にはTrue
    if is_los:
      pass_loss: float = 0.0
      shadowing: float = awgn.calculate(shadowing_standard_deviation) # 短区間のフェージング
      ...
      if rx_power > receiver_sensitivity_threthold: #受信電力が受信感度より大きいかどうか
